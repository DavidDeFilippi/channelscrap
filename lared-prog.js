...
const doScrap = async () => {
  ...
  try {
    ...
    for (let i = 0; i < programas.length; i++) {
      ...
      programacion.push({id: 'lared', programa: programas[i], hora: d, horaNormal: new Date(d).toLocaleTimeString().slice(0, -3), updated: new Date().getTime()});
    }

    if(programacion.length > 0){
      const jsonData = JSON.stringify(programacion);

      fs.writeFileSync("/home/deltafoxtrot/flytvtools/"+"redred.json", jsonData);
