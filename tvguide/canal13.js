...
const doScrap = async () => {
  ...
  try {
    ...
    let programacionElement = await page.$('.programacion-dia');

    console.log(programacionElement);

    for (let i = 0; i < programacionElement.length; i++) {
    ...
    //   ".programacion-dia > .programas > .hora",
    //   // ".programacion-dia:nth-child(3)",
    //   els => els.map(e => e.html)
    // );
    ...
    //   programacion.push({ id: 'canal13', programa: programas[i], hora: d, horaNormal: new Date(d).toLocaleTimeString().slice(0, -3), updated: new Date().getTime() });
    // }

    // if(programacion.length > 0){
    //   const jsonData = JSON.stringify(programacion);

    //   // fs.writeFileSync("/home/deltafoxtrot/tvguidejson/" + "tareaescuelita13.json", jsonData);
