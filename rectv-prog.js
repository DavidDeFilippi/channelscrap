...
const doScrap = async () => {
  ...
  try {
    ...
    const url = 'https://tvdaldia.cl/cartelera-tv/rec/';
    ...
    for (let i = 0; i < programas.length; i++) {
      ...
      programacion.push({id: 'rectv', programa: programas[i], hora: d, horaNormal: new Date(d).toLocaleTimeString().slice(0, -3), updated: new Date().getTime()});
    }

    if(programacion.length > 0){
      const jsonData = JSON.stringify(programacion);

      fs.writeFileSync("/home/deltafoxtrot/flytvtools/"+"old13.json", jsonData);
