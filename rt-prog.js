...
const doScrap = async () => {
  ...
  try {
    ...
    const url = 'https://actualidad.rt.com/en_vivo';
    ...
    const programas = await page.$$eval(
      ".EnVivoSchedule-itemTitle",
      els => els.map(e => e.textContent)
    );

    const horarios = await page.$$eval(
      ".EnVivoSchedule-itemTime",
      els => els.map(e => e.textContent)
    );
    ...
    for (let i = 0; i < programas.length; i++) {
      ...
      programacion.push({id: 'rt', programa: programas[i], hora: d, updated: new Date().getTime()});
    }
