...
const doScrap = async () => {
  ...
  try {
    ...
    for (let i = 0; i < programas.length; i++) {
      ...
      programacion.push({id: 'tvmas', programa: programas[i], hora: d, horaNormal: new Date(d).toLocaleTimeString().slice(0, -3), updated: new Date().getTime()});
    }
    ...
    if(programacion.length > 0){
      ...
      fs.writeFileSync("/home/deltafoxtrot/flytvtools/"+"masymas.json", jsonData);
